...
sql-cli 是一个基于 JDBC 的多数据库 CLI 工具，支持 MySQL、Oracle、PostgreSQL、SQLite、达梦等数据库。专为 AI 编程助手（如 Claude Code）设计。

## 项目状态

**Beta 测试阶段**。未发布到 npm，通过 GitHub 管理和分发。package.json 中的 npm 配置是为未来稳定版本准备的。
...
│   ├── QueryCommand            # sql-cli query "SQL" -c name
│   ├── ExecCommand             # sql-cli exec "SQL" -c name [--confirm]
│   ├── MetaCommand (group)     # sql-cli meta dbs/tables/columns/indexes/views
...
├── executor/
│   ├── QueryExecutor           # SELECT 执行，输出格式化
│   ├── MetaExecutor            # 元数据查询（DatabaseMetaData）
...
- **命令模式**：所有 CLI 命令 `implements Runnable`，使用 Picocli 注解（`@Command`, `@Option`, `@Parameters`）
- **方言工厂**：`DialectFactory.getDialect(type, config)` 按优先级解析：内置 → 自定义类型 → GenericDialect
- **配置缓存**：`ConfigManager.load()` 首次读取后缓存在内存
- **安全级别**：连接级可覆盖全局配置，strict/normal/none 三档
- **自动行数限制**：SELECT 默认限制 500 行，通过 dialect 的 `wrapLimit()` 实现
...
3. 在 `ConfigManager` 的默认驱动映射中添加
4. 添加 `src/test/java/com/sqlcli/dialect/` 下的测试

## 测试

- 测试目录：`src/test/java/com/sqlcli/`
- 已有测试：SqlAnalyzerTest, EncryptionServiceTest, {Mysql,Oracle,Sqlite}DialectTest, DialectFactoryTest, {Csv,Json,Markdown}FormatterTest
- 运行：`./gradlew test`
